...
/**
 * @brief       getData checks x, y position and returns number of fingers on
 * screen
 *
 * @param       uint16_t *xPos
...
uint8_t Touch::getData(uint16_t *xPos, uint16_t *yPos, uint8_t *z)
{
    ...
    if (!getTouchData(&_touchReport))
        return 0;

    // Scale it to fit the screen.
    scale(&_touchReport, E_INK_WIDTH - 1, E_INK_HEIGHT - 1, false, true, true);
 ...
 */
bool Touch::getTouchData(struct cypressTouchData *_touchData)
{
    ...
    if (!readI2CRegs(CYPRESS_TOUCH_BASE_ADDR, _regs, sizeof(_regs)))
        return false;
...
/**
 * @brief       getPowerState gets current power mode of the touchscreen.
 *
 ...
 */
bool Touch::loadBootloaderRegs(struct cyttspBootloaderData *_blDataPtr)
{
    // Bootloader temp. registers array.
    uint8_t _bootloaderData[16];

    // read bootloader data and save it into dedicated struct.
    if (!readI2CRegs(CYPRESS_TOUCH_BASE_ADDR, _bootloaderData, 16))
        return false;
 ...
 */
bool Touch::setSysInfoMode(struct cyttspSysinfoData *_sysDataPtr)
{
 ...
 */
bool Touch::setSysInfoRegs(struct cyttspSysinfoData *_sysDataPtr)
{
...
bool Touch::readI2CRegs(uint8_t _cmd, uint8_t *_buffer, int _len)
{
    ...
    while (_len > 0)
    {
        ...
        Wire.requestFrom(CYPRESS_TOUCH_I2C_ADDR, _i2cLen);
