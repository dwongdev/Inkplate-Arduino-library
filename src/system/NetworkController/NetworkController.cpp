 ...
 */
bool NetworkController::connectWiFi(const char *ssid, const char *pass, int timeout, bool printToSerial)
{
    // Init WiFi
 ...
 * @return      True if successful, false if failed
 */
bool NetworkController::getNTPEpoch(time_t *timeEpoch, int timeZone, char *ntpServer, int daylightSavingsOffsetHours)
{
 ...
 * @return      True if successful, false if failed
 */
bool NetworkController::getNTPDateTime(tm *dateTime, int timeZone, char *ntpServer, int daylightSavingsOffsetHours)
{
    // If WiFi is not connected, we can't get the epoch
    if (!isConnected())
    {
 ...
 */
char *NetworkController::getHostFromURL(const char *urlToGetHostFrom)
{
 ...
 */
char *NetworkController::getPathToResourceFromURL(const char *urlToGetPathToResourceFrom)
{
    // Check if url is HTTP or HTTPS
    int offsetToGetToBeginningOfHost = 0;
 ...
 */
uint8_t *NetworkController::downloadFileHTTPS(const char *url, int32_t *defaultLen)
{
    if (!isConnected())
        return NULL;
 ...
 * @return      pointer to buffer that holds downloaded file
 */
uint8_t *NetworkController::downloadFile(WiFiClient *s, int32_t len)
{
    if (!isConnected())
        return NULL;
 ...
 * @return      pointer to buffer that holds downloaded file
 */
uint8_t *NetworkController::downloadFile(const char *url, int32_t *defaultLen)
{
    if (!isConnected())
        return NULL;
 ...
 *
 */
void NetworkController::setFollowRedirects(followRedirects_t f)
{
