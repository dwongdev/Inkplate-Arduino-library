/*
  ...
  Don't have "Soldered Inkplate13SPECTRA" option? Follow our tutorial and add it: https://soldered.com/documentation/inkplate/13SPECTRA/quick-start-guide/
...
void setup() {
  inkplate.begin(); // Initialize the Inkplate display
  ...
  if (getWeather(latitude, longitude, temperature, weatherDesc, timeStr)) {
    ...
    inkplate.drawTextBox(50, 60, 800, 900, snarkySummary.c_str(), 1, &FreeMonoBold18pt7b, 58, false, 32);
    inkplate.display(); // Push the buffer to the screen
  } else {
    // Handle weather data fetch failure
    inkplate.println("Failed to get weather data.");
    inkplate.display();
  }
