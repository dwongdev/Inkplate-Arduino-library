...
{
    // Main info, city, temperature, weather description
    //inkplate.fillRect(0, 0, 1600, 291, 1);
    inkplate.drawRect(0, 0, 1600, 291, 0);

    // Battery info, last refresh, username
    inkplate.fillRect(1033, 0, 567, 291, 0);

    // Additional weather info
    inkplate.fillRect(0, 291, 467, 679, 0);

    // Temperature & Precipitation graph
    //inkplate.fillRect(467, 291, 1067, 611, 1);
    //inkplate.drawRect(467, 291, 1067, 611, 1);

    // Weekly forecast
    inkplate.fillRect(467, 970, 1133, 230, 0);
}
...
void Gui::drawTemperaturePrecipGraph(WeatherData *weatherData, NetworkFunctions::UserInfo *userInfo)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        inkplate.fillRect(x + 20, y + 10, 15, barHeight - 10, 4);
...
void Gui::displayWeatherData(WeatherData *weatherData, NetworkFunctions::UserInfo *userInfo)
{
    ...
    int yUser = 60;
    inkplate.drawBitmap(1100, 25, getBatteryIcon(batteryLevel), 48, 48, 1);
    ...
    {
        inkplate.drawBitmap(iconX, iconY, icon_s_moon, 48, 48, 0);
        inkplate.setCursor(iconX + 70, iconY + 35);
        inkplate.setTextColor(0);
        inkplate.setFont(&FreeSansBold24pt7b);
