/*
   Inkplate6_Internal_IO_Expander example for Soldered Inkplate 6
   For this example you will need only a micro USB cable, Inkplate6, 330 Ohm resistor and LED diode.
   Select "e-radionica Inkplate 6" or "Soldered Inkplate6" from Tools -> Board menu.
   Don't have "e-radionica Inkplate 6" or "Soldered Inkplate6" option? Follow our tutorial and add it:
...
#error "Wrong board selection for this example, please select e-radionica Inkplate 6 or Soldered Inkplate6 in the boards menu."
#endif
...
void setup()
{
    ...
    display.internalIO.pinMode(LED_PIN, OUTPUT); // Set P1-7 (or GPB7) to output. On that pin, we sholud connect LED with current
