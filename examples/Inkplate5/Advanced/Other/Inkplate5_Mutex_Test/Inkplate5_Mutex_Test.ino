/**
 **************************************************
 * @file        Inkplate5_Mutex_Test.ino
 * @brief       Two-task display mutex test for Inkplate 5.
 *
 ...
 *
 *              Left task:  bouncing square on the left half  (x 0..479)
 *              Right task: bouncing square on the right half (x 480..959)
 *
...
#error "Wrong board selection — select Soldered Inkplate5 in the boards menu."
#endif
...
void taskLeft(void *pvParameters)
{
    ...
    {
        display.displayLock();
        display.fillRect(0, 0, 480, display.height(), WHITE);
        display.fillRect(190, y, SQ, SQ, BLACK);
...
void taskRight(void *pvParameters)
{
    ...
    {
        display.displayLock();
        display.fillRect(480, 0, 480, display.height(), WHITE);
        display.fillRect(670, y, SQ, SQ, BLACK);
        display.partialUpdate(false, true);
        display.displayUnlock();
...
void setup()
{
    ...
    display.drawFastVLine(480, 0, display.height(), BLACK);
    display.display();
