...
{
    //main info
    inkplate.fillRect(0, 0, 600, 160, 0);

    // user info
    // inkplate.fillRect(530, 0, 270, 155, 0);

    //graph
    inkplate.fillRect(0, 160, 600, 295, 7);

    // weekly
    inkplate.fillRect(0, 450, 600, 150, 0);
}
...
void Gui::displayWeatherData(WeatherData *weatherData, NetworkFunctions::UserInfo *userInfo)
{
    ...
    inkplate.drawBitmap(10, 5, icon_s_gray, 48, 48, 7);
    ...
    inkplate.setFont(&FreeSans12pt7b);
    inkplate.drawBitmap(10, 106, getWeatherIcon(weatherData->weatherCode), 48, 48, 7);
    ...
    inkplate.drawBitmap(390, 10, getBatteryIcon(batteryLevel), 48, 48, 7);

    yUser += 30;

    inkplate.setCursor(440, yUser + 2);
    inkplate.print(batteryLevel);
    inkplate.println("%");

    yUser += 40;
    inkplate.setCursor(390, yUser);
    ...
    int startX = 30;                      // Starting x-position for the weekly forecast
