/*
   Inkplate4TEMPERA_Peripheral_Mode example for Soldered Inkplate 4 TEMPERA
   ...
   Using this sketch, you don't have to control the e-paper using compiled Arduino code.
   Instead, you can send UART commands. 
   This gives you flexibility so you can use Inkplate 4TEMPERA on any platform!

   Because it uses UART, it's little bit slower and it's not recommended to send a bunch of
   drawPixel commands to draw an image. Instead, load bitmaps and pictures on SD card and load image from SD.
...
// Create Inkplate object in 1 bit (BW) mode
