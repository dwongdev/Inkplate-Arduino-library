/*
   Inkplate5V2_RTC_Interrupt_Alarm example for Soldered Inkplate 5 V2
   ...
   Inkplate using partial updates and how to handle interrupt. 
   NOTE: Partial update is only available on 1 Bit mode (BW) and it is not recommended to use it on first refresh
   after power up. It is recommended to do a full refresh every 5-10 partial refresh to maintain good picture quality.
...
void setup()
{
    ...
    // display.rtc.SetAlarm(10, 25, 6, 21, 2); // Set alarm 10 seconds from now
