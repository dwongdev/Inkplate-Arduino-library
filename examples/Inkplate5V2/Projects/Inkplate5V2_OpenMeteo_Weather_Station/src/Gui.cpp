...
void Gui::drawBackground()
{
    ...
    // battery info
    inkplate.fillRect(930, 0, 350, 200, 0);

    // add info
    inkplate.fillRect(0, 200, 480, 375, 0);

    // graph info
    inkplate.fillRect(400, 200, 880, 400, 7);
    inkplate.drawRect(400, 200, 880, 400, 0);

    // weekly
    inkplate.fillRect(400, 575, 880, 170, 0);
}
...
void Gui::displayWeatherData(WeatherData *weatherData, NetworkFunctions::UserInfo *userInfo)
{
    ...
    inkplate.drawBitmap(950, 5, getBatteryIcon(batteryLevel), 48, 48, 7);

    inkplate.setCursor(1000, yUser);
    inkplate.print(batteryLevel);
    inkplate.println("%");

    yUser += 45;
    inkplate.setCursor(950, yUser);
    ...
    int startX = 420;                      // Starting x-position for the weekly forecast
    int startY = 600;                      // Starting y-position for the weekly forecast
    int iconSize = 48;                     // Size of the icon
    int margin = 5;                        // Margin between elements
    int dayWidth = iconSize + margin + 75; // Space for icon + margin + text width
    ...
    {
        inkplate.fillRect(0, 600, 400, 150, 7);
