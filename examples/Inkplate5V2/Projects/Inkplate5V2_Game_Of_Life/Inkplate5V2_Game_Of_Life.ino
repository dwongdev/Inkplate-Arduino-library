/**
 **************************************************
 * @file        Inkplate5v2_Game_Of_Life.ino
 * @brief       Conway’s Game of Life animation (community example) for Soldered Inkplate 5v2.
 *
 * @details     Community-created example that runs Conway’s Game of Life on the
 *              Inkplate 5v2 e-paper display. A random initial grid is generated,
...
char grid_a[(E_INK_WIDTH / MIN_CELLSZ) * (E_INK_HEIGHT / MIN_CELLSZ)],
    grid_b[(E_INK_WIDTH / MIN_CELLSZ) * (E_INK_HEIGHT / MIN_CELLSZ)];
...
void randomize()
{
    ...
    cols = E_INK_WIDTH / cell_size;
    rows = E_INK_HEIGHT / cell_size;

    cell_maxage = (cell_size / 2) - 1;
    // Compute the "extra" space not covered by this grid, offset by half
    offset_x = (E_INK_WIDTH - cols * cell_size) / 2;
    offset_y = (E_INK_HEIGHT - rows * cell_size) / 2;
