/**
 **************************************************
 * @file        Inkplate5v2_Grayscale.ino
 * @brief       Grayscale (3-bit) drawing demo using Adafruit GFX on Soldered Inkplate 5v2.
 *
 * @details     Demonstrates drawing and text rendering on the Inkplate 5v2 in
...
// Other option is BW mode, which is demonstrated in next example "Inkplate5V2_Black_And_White"
...
void loop()
{
    ...
    display.clearDisplay();
    display.drawFastHLine(239, 359, 798, 0); // Arguments are: starting X, starting Y, length, color
    ...
    display.clearDisplay();
    display.drawFastVLine(638, 93, 532, 0); // Arguments are: starting X, starting Y, length, color
    ...
    display.clearDisplay();
    display.drawRect(372, 159, 540, 400, 0); // Arguments are: start X, start Y, size X, size Y, color
    ...
    // Draw filled black rectangle at X = 372, Y = 159, size of 540x540 pixels in gray color
    display.clearDisplay();
    display.fillRect(372, 159, 540, 400, 4); // Arguments are: start X, start Y, size X, size Y, color
    ...
    display.clearDisplay();
    display.drawRoundRect(372, 159, 540, 400, 13, 0); // Arguments are: start X, start Y, size X, size Y, radius, color
    ...
    // Draw filled random colored rectangle at X = 372, Y = 159, size of 540x400 pixels and radius of 13 pixels
    display.clearDisplay();
    display.fillRoundRect(372, 159, 540, 400, 13, 0); // Arguments are: start X, start Y, size X, size Y, radius, color
    ...
    display.clearDisplay();
    display.drawTriangle(405, 585, 638, 119, 871, 585, 0); // Arguments are: X1, Y1, X2, Y2, X3, Y3, color
    displayCurrentAction("Drawing triangle");
    display.display();
    delay(DELAY_MS);

    // Draw filled triangle inside simple triangle (so no display.clearDisplay() this time)
    display.fillTriangle(505, 518, 638, 252, 771, 518, 0); // Arguments are: X1, Y1, X2, Y2, X3, Y3, color
    ...
    display.clearDisplay();
    display.drawElipse(133, 266, E_INK_WIDTH / 2, E_INK_HEIGHT / 2, 0);
    ...
    display.clearDisplay();
    display.fillElipse(133, 266, E_INK_WIDTH / 2, E_INK_HEIGHT / 2, 0);
