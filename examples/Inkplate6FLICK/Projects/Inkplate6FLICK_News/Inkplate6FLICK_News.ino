...
#error "Wrong board selection for this example, please select Soldered Inkplate 6FLICK in the boards menu."
#endif
...
void drawNews(struct news *entities)
{
    ...
    for (int i = 0; i < maxBoxes && entities[i].title != nullptr && entities[i].description != nullptr; i++)
    {
        ...
        // Draw the title
        inkplate.drawTextBox(leftMargin, y0, inkplate.width() - rightMargin+50, y0 + 70, entities[i].title, 1, &GT_Pressura16pt7b, 26, false, 12);

        // Draw the description
        inkplate.drawTextBox(leftMargin, y0 + 65, inkplate.width() - rightMargin, y1, entities[i].description, 1, &Inter12pt7b, 20, false, 10);
    }
}
