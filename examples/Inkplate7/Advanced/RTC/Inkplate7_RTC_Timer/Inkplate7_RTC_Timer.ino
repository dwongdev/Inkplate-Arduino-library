/*
    Inkplate7_RTC_Timer example for Soldered Inkplate 7
...
// Next 3 lines are a precaution, you can ignore those, and t1he example would also work without them
...
void loop()
{
    ...
    display.setCursor(88, 120);                                   // Set position of the text
    printTime(hour, minutes, seconds, day, weekday, month, year); // Print the time on screen
    display.display();                                            // Show on Inkplate

    delay(15000); // Delay between refreshes (15 seconds)
}
               ...
               uint16_t _year)
{
    ...
    if (display.rtc.CheckTimerFlag()) // Check if timer event has occurred
    {
        ...
        display.rtc.DisableTimer();      // Disable timer if you want to make it one time only. Is you want to be repeatable,
                                        // comment this line

        display.setCursor(200, 230);     // Set new position for cursor
        display.print("Timer!");
    }
}
