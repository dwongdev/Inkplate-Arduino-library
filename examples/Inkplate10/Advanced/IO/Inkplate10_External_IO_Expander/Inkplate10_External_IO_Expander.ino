/*
   Inkplate10_External_IO_Expander example for Soldered Inkplate 10
   ...
   Connect resistor to P1-7 pin (GPB7 pin on older Inkplates with MCP23017) on IO Expander 2 header at bottom rihgt side
   on the backside (component side) of Inkplate. You will have to connect one side of 330 Ohm resistor to P1-7 (GPB7),
   then other side to anode of LED and finally, cathode pin of LED to GND.
...
void setup()
{
    ...
    display.externalIO.pinMode(LED_PIN, OUTPUT); // Set P1-7 (or GPB7) to output. On that pin, we sholud connect LED with current limiting
