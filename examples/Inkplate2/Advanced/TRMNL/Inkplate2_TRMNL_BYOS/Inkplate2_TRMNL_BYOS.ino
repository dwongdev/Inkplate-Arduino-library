/**
 **************************************************
 * @file        Inkplate2_TRMNL_BYOS.ino
 * @brief       TRMNL BYOS client example for Soldered Inkplate 2.
 *
 * @details     Connects Inkplate 2 to WiFi, registers with a TRMNL-compatible
 *              BYOS server via /api/setup, then polls /api/display on a loop,
 *              drawing whatever image the server returns and deep-sleeping
 *              between refreshes.
 *
 *              NOTE: Inkplate 2 is a 3-color (black/white/red) panel and has
 *              no fast partial-refresh call, so unlike the monochrome boards
 *              this sketch prints WiFi-connect progress dots to Serial only,
 *              instead of to the screen. Images fetched via image.draw()
 *              are automatically dithered to the 3-color palette. Given the
 *              panel's small, low resolution, point your BYOS template at a
 *              matching small image.
 *
 * Requirements:
...
void setup()
{
    ...
    {
        delay(1000);
        Serial.print('.'); // No fast partial refresh on this panel, so progress goes to Serial only
    }
